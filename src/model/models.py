...
class PixelNeRFNet(torch.nn.Module):
    def __init__(self, conf, stop_encoder_grad=False, combine_type="average"):
        ...
        self.mlp_coarse = make_mlp(conf["mlp_coarse"], d_in, d_latent, d_out=d_out, combine_type=combine_type, positional_encoder = self.code)
        self.mlp_fine = make_mlp(
            conf["mlp_fine"], d_in, d_latent, d_out=d_out, allow_empty=True, combine_type = combine_type, positional_encoder = self.code, 
        )
